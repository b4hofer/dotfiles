...
-- For keysyms (brightness, volume etc.)
...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    -- Screenshot
    , ((0, xK_Print), spawn "scrot -s '/tmp/screenshot_%Y-%m-%d_%H-%M-%S.png' -e 'xclip -selection clipboard -target image/png -i $f'")

    -- Launch KeepassXC
    , ((modm              , xK_F8   ), spawn "keepassxc")
    ]
    ++
    ...
    -- Using PhysicalScreens for correct order
    --
    [((m .|. modm, key), f sc)
...
main = do
    ...
    spawn "xsetwacom set \"Wacom Intuos PT S 2 Pen stylus\" MapToOutput 2304x1440+1136+0"
